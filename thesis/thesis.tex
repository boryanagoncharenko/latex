...
single node. The corresponding construct in the system is Expression. Similarly, the specific subclasses
...
In the modeling grammar, when a Pattern Descriptor refers to multiple nodes, the presence of Node
Descriptor denotes the existence of CSS Node and the Node Relation specifies how the Nodes are
connected in the Pattern Descriptor. The Node Descriptor can also impose constraints on the type or
the properties of the matched node, however, its presence requires the existence of a node.

As described in the domain analysis section, each Pattern Descriptor has one of more anchors. These
are the specific nodes that need to be iterated. The Node Query Expression is used to describe
parts of the pattern that follow the anchors. a bit more than simply checking for existence. It is
used to ensure that context of the node, however, unlike the Node Descriptor, For example, it is
used to for finding the number of occurrences, whether a list of nodes exist in a specific context.



For this reason, the candidate constructs
...
construct. Typically, ontological analysis is performed through comparing the modeling grammar to
the ontology. In the conducted analysis, every structure in the abstract syntax is mapped to an
ontological construct. 

However, the particular way this analysis is performed is comparing the whole system to the designed
ontology. Because of this approach, there are a number of classes and even modules that do not map
to any ontological construct. For example, the presence of a Type Checker does not have an
ontological counterpart. In fact, the Type Checker is used to ensure the correctness of the Style
Guide. However, the ontology does not consider its correctness.
...
and supported with argumentation why they should not be considered anomalies. Having the analysis in
